...
MyStringImpl::MyStringImpl( const unsigned char* val ):
...
MyStringImpl& MyStringImpl::operator=( const unsigned char * rhv )
{
...
void MyStringImpl::replace( const MyString& inWhat, const MyString& inFor )
{
    ...
    while( std::string::npos != inWhatPosition )
    {
        ...
        inWhatPosition = this->m_value.find( inWhat.string() );
    }
}

const std::string& MyStringImpl::string() const
{
    return this->m_value;
}
...
const bool MyStringImpl::empty() const
{
